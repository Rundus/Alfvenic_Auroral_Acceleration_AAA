...
class WaveFieldsClasses3D:

    k_vectors = np.array(
        [data_dict_wavescale['k_mu'][0], data_dict_wavescale['k_chi'][0], data_dict_wavescale['k_phi'][0]]).T
    wave_pos_vector = np.array(
        [data_dict_wavescale['mu_w'][0], data_dict_wavescale['chi_w'][0], data_dict_wavescale['phi_w'][0]]).T
    ...
    def InWaveChecker3D(self, inputs):
        tme_idx, eval_pos, wave_pos, k, h, lmb_e = inputs
