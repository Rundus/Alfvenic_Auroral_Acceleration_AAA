...
def distribution_generator():
    ...
    def equations_of_motion(t, S, deltaT):
        ...
        DvmuDt = (- (uB / stl.m_e) * (dB_dipole_dmu(S[0], S[1]) / h_factors[0](S[0], S[1])) - (stl.q0 / stl.m_e) * WaveFieldsClasses().field_generator(time=SimToggles.RK45_tspan[1] -deltaT-t, eval_pos=[S[0],S[1],S[2]],type='epara'))
    ...
    data_dict_output = {
        ...
        'particle_mu': [np.array(particle_mu_tracker), {'DEPEND_0':'time_eval','DEPEND_1':'v_perp_range','DEPEND_2':'v_mu_range','UNITS': None, 'LABLAXIS': '&mu;', 'VAR_TYPE':'data'}],
        'particle_vel_mu' : [np.array(particle_vel_mu_tracker), {'DEPEND_0':'time_eval','DEPEND_1':'v_perp_range','DEPEND_2':'v_mu_range','UNITS': 'm/s', 'LABLAXIS': 'V!B&mu;!N', 'VAR_TYPE':'data'}],
        'particle_alt_along_mu': [np.array(particle_mu_alt_tracker), {'DEPEND_0':'time_eval','DEPEND_1':'v_perp_range','DEPEND_2':'v_mu_range', 'UNITS': 'km', 'LABLAXIS': '&mu;!N', 'VAR_TYPE': 'data'}],
        'particle_vel_perp': [np.array(particle_vel_perp), {'DEPEND_0': 'time', 'UNITS': 'm/s', 'LABLAXIS': 'V!B&perp;!N', 'VAR_TYPE': 'data'}],
        'Distribution': [np.array(Distribution), {'DEPEND_0':'time_eval','DEPEND_2':'v_perp_range_eV','DEPEND_1':'v_mu_range_eV','UNITS':'m!A-6!Ns!A-3!N','LABLAXIS':'Distribution Function','VAR_TYPE':'data'}],
        'v_perp_range': [np.array(DistributionToggles.vel_space_perp_range), {'UNITS':'m/s','LABLAXIS':'V!B&perp;!N'}],
        'v_mu_range': [np.array(DistributionToggles.vel_space_mu_range), {'UNITS':'m/s','LABLAXIS':'V!B&parallel;!N'}],
        'v_perp_range_eV': [np.array([math.copysign(0.5*(stl.m_e/stl.q0)*np.square(val),val) for val in DistributionToggles.vel_space_perp_range]), {'UNITS': 'eV', 'LABLAXIS': 'V!B&perp;!N'}],
        'v_mu_range_eV': [np.array([math.copysign(0.5*(stl.m_e/stl.q0)*np.square(val),val) for val in DistributionToggles.vel_space_mu_range]), {'UNITS': 'eV', 'LABLAXIS': 'V!B&parallel;!N'}],
    }
