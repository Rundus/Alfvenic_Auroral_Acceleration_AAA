...
def distribution_generator():
    ...
    # --- PREPARE OUTPUTS ---
    Distribution = np.zeros(shape=(len(DistributionToggles.RK45_Teval),
    ...
    ########################################
    sizes = [len(DistributionToggles.RK45_Teval),len(DistributionToggles.pitch_range),len(DistributionToggles.energy_range)]
    ...
    for tmeIdx, ptchIdx, engyIdx in tqdm(product(*[range(val) for val in sizes])):
        ...
        s0 = [DistributionToggles.u0_obs,
              DistributionToggles.chi0_obs,
              v_mu0,
              v_perp0]
